...
async def _impl_call_mcp_tool(
    ...
    try:
        ...
        print(f"Error calling MCP tool '{name}' on server '{server_name}': {err}")
        ...
        print(f"Attempting to refresh token for server '{server_name}' and retry tool call '{name}'...")
        ...
        print("after connect to server")     
        fresh_session = mcp_client.servers[server_name]          
        print(f"Retrying tool call '{name}' with fresh connection channel...")
